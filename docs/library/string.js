...
function convertHTMLtoUTF8(pString)
{
   ...
   // so if you see blocks instead of german letters, the
   // problem is caused by the different code tables.
        pString = replaceString(pString,'&auml;','ä');
        pString = replaceString(pString,'&ouml;','ö');
        pString = replaceString(pString,'&uuml;','ü');
        pString = replaceString(pString,'&Auml;','Ä');
        pString = replaceString(pString,'&Ouml;','Ö');
        pString = replaceString(pString,'&Uuml;','Ü');
        pString = replaceString(pString,'&szlig;','ß');
	return pString
}
...
function normalizeSearchWords(pString)
{
   ...
   // problem is caused by the different code tables.
        pString = convertHTMLtoDOS(pString);
        pString = pString.toLowerCase();
        pString = replaceString(pString,'ä','ae');
        pString = replaceString(pString,'ö','oe');
        pString = replaceString(pString,'ü','ue');
        pString = replaceString(pString,'Ä','ae');
        pString = replaceString(pString,'Ö','oe');
        pString = replaceString(pString,'Ü','ue');
        pString = replaceString(pString,'ß','ss');
        pString = pString.replace(/[ ]+/," ");
		pString = pString.replace(/[^a-z0-9 ]/,"");
		//alert("normalized Search Words="+pString);
		return pString
